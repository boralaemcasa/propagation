  ...
  while i <= length(s) do
    ...
    begin
      if not (s[i + 1] in ['0'..'9', '(']) then
        if copy(s, i + 1, 3) <> 'gcd' then
        begin
          Application.MessageBox('gcd or digit or ( expected after + or - or * or /', 'Syntax error', MB_ICONEXCLAMATION);
          memo.free;
          comma.free;
          exit;
        end;
    ...
    begin
      if s[i + 1] in [',', ')', '*', '/', '^'] then
      begin
        Application.MessageBox('+, - or digit expected after ( or comma', 'Syntax error', MB_ICONEXCLAMATION);
        memo.free;
        comma.free;
        exit;
      end;
    ...
    else if copy(s, i, 3) = 'gcd' then
    begin
      if s[i + 3] <> '(' then
      begin
        Application.MessageBox('( expected after gcd', 'Syntax error', MB_ICONEXCLAMATION);
        memo.free;
        comma.free;
        exit;
      end;
