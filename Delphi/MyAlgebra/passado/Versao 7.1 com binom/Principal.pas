...
begin
  ...
  begin
    ...
    begin
      j := i + 3;
      repeat
        if (j < memo.Count) and (memo[j - 1] = '/') and (memo[i - 1] = memo[j]) and ((j + 1 >= memo.count) or (memo[j + 1] <> '^')) then // x^3 / x
        begin
        ...
        if j > memo.count - 1 then
          break;
      until (memo[j] = '+') or (memo[j] = '-') or (memo[j] = '^') or (memo[j] = '/');

      if (i >= 3) and (memo[i - 2] = '/') and (memo[i - 1] = memo[i - 3]) then // x / x^3
      begin
        Memo[i - 3] := '1';
        ...
        repeat
          ...
          begin
            ...
            if (j >= 1) and (memo[j - 1] = FracValida(memo[j - 1])) then
              Memo.Delete(j)
            else if (j + 1 < memo.count) and ((memo[j + 1] <> '+') and (memo[j + 1] <> '-') and (memo[j + 1] <> '^') and (memo[j + 1] <> '*')) then
              Memo.Delete(j)
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            s := WithParenthesisProcess(true, s, LOGFILE);
            if pos('/', s) > 0 then // only integers allowed to binom
              s := '1';
            ...
            begin
              ...
              s := binom(s, WithParenthesisProcess(true, t, LOGFILE));
            end;
...
begin
  ...
  if t <> '' then
    result := result + '=' + WithParenthesisProcess(false, t, LOGFILE);

  repeat
    s := denominator(Result);
    if (s = '') or (s = fracValida(s)) then
      break;
  ...
  begin
    ...
    s := StringReplace(s, #32, '', [rfReplaceAll]);
    Result := WithParenthesisProcess(true, s, LOGFILE);
    if t <> '' then
      result := result + '=' + WithParenthesisProcess(true, t, LOGFILE);
  end;
