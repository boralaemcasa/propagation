...
//trim alphabet
  i := 1;
  while i <= length(s) do
    if s[i] in ['0'..'9', '+', '-', '*', '/', '^', 'g', 'c', 'd', '(', ')', ',', 'a', 'b', 's', '=', 'f', 'r'] then
      inc(i)
    else delete(s, i, 1);

  if not (s[1] in ['0'..'9', '(', '+', '-']) then
    if (copy(s, 1, 3) <> 'gcd') and (copy(s, 1, 3) <> 'abs') then
      if copy(s, 1, 4) <> 'frac' then
      begin
        Application.MessageBox('+, -, abs or gcd or frac or digit or ( expected as first character', 'Syntax error', MB_ICONEXCLAMATION);
        exit;
      end;
  ...
  while i <= length(s) do
    ...
    begin
      if not (s[i + 1] in ['0'..'9', '(']) then
        ...
        if copy(s, 1 + 1, 4) <> 'frac' then
          begin
            Application.MessageBox(PChar('abs or gcd or frac or digit or ( expected after ' + s[i]), 'Syntax error', MB_ICONEXCLAMATION);
            memo.free;
            comma.free;
            frac.free;
            exit;
    ...
    begin
      ...
      begin
        ...
        if memo.count >= 1 then
          ...
          else if memo[memo.Count - 1] = 'frac' then
            frac.Add(inttostr(j));
      end;
      t := copy(s, 1, i-1);
      if t <> '' then
  ...
  repeat
    ...
    begin
      ...
      begin
        ...
        Memo[i+1] := NoParenthesisProcess(s, true);
        jump := true;
        while Memo[i + 2] <> ')' + inttostr(max) do
          Memo.Delete(i + 2);
        Memo.Delete(i+2);
