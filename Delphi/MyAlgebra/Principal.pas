...
function AlgebricalInverse(s: string): string;
...
begin
  ...
  hora := now - hora;
  caption := inttostr(round(hora * 24 * 60 * 60)) + ' seconds';
end;
...
begin
  ...
  begin
    if i + 1 < memo.count then
      if memo[i] = '^' then
        ...
        else if (memo[i - 1] = '1') or (memo[i - 1] = '-1') then
        begin
          memo.Delete(i);
          memo.Delete(i);
          i := 0;
  ...
  begin
    if i + 1 < memo.count then
      if (Memo[i] = '*') and ((i < 2) or (copy(memo[i - 2], length(memo[i - 2]), 1) <> '^'))
        and (copy(memo[i + 1], length(memo[i + 1]), 1) <> '^') then
        ...
        else if ((i + 2 >= memo.count) or (memo[i + 2] <> '/')) and ((i <= 2) or (memo[i - 2] <> '/')) then
        begin
    ...
    begin
      ...
      repeat
        if (j < memo.Count) and (memo[j - 1] = '/') and (memo[i - 1] = memo[j]) and ((j + 1 >= memo.count) or (memo[j + 1] <> '^'))
          and (memo[i + 2] <> '+') and (memo[i + 2] <> '-') and (memo[i + 1] = fracValida(memo[i + 1])) then // x^3 / x
        begin
      ...
      j := i - 3;
      repeat
        if (j >= 0) and (memo[i - 2] = '/') and (memo[i - 1] = memo[j]) and (memo[j + 1] <> '^')
          and (memo[i + 1] = fracValida(memo[i + 1])) and ((j = 0) or (memo[j - 1] <> '^')) then // x / x^3
        begin
          Memo[j] := '1';
          if memo[i + 1] = '2' then
          begin
      ...
      while (j >= 0) and (memo[j] <> '+') and (memo[j] <> '-')
        and (memo[i - 4] <> '+') and (memo[i - 4] <> '-') and (memo[i - 3] <> '+') and (memo[i - 3] <> '-') and (memo[i + 1] = fracValida(memo[i + 1])) do
      begin
        if (j >= 0) and (memo[i - 2] = '/') and (memo[i - 1] = memo[j]) and (memo[j + 1] = '^') and (memo[j + 2] = fracValida(memo[j + 2])) then // x^2 / x^3
        begin
  ...
  begin
    if Memo[i] = '/' then
      ...
      begin
        j := i - 1;
        repeat
          if (memo[j] = memo[i + 1]) and ((i < 2) or (memo[i - 2] <> '^')) then // x / x
          begin
            Memo[j] := '1';
            Memo.Delete(i);
            Memo.Delete(i);
  ...
  begin
    ...
    begin
      j := i + 2;
      while (j < memo.count) and (memo[j] <> '+') and (memo[j] <> '-') do
      begin
        if (i >= 1) and (j < memo.Count) and (memo[i - 1] = memo[j]) and ((j + 1 >= memo.count) or (memo[j + 1] <> '^')) and (memo[i + 1] = fracValida(memo[i + 1])) then // x^3 x
        begin
          s := Soma(Memo[i + 1], '1');
          if s <> '1' then
            memo[i - 1] := memo[i - 1] + '^' + s;
      ...
      begin
        if (i >= 1) and (j >= 0) and (memo[i - 1] = memo[j]) and (memo[j + 1] <> '^')
          and (memo[i + 1] = fracValida(memo[i + 1])) and ((i < 2) or (memo[i - 2] <> '/'))
          and ((j < 1) or (memo[j - 1] <> '^')) then // x x^3
        begin
          s := Soma(Memo[i + 1], '1');
          if s <> '1' then
            memo[j] := memo[j] + '^' + s;
      ...
      while (j < memo.count) and (memo[j] <> '+') and (memo[j] <> '-') do
      begin
        if (i >= 1) and (j + 1 < memo.Count) and (memo[i - 1] = memo[j]) and (memo[j + 1] = '^')
          and (memo[i + 1] = fracValida(memo[i + 1])) and (memo[j + 2] = fracValida(memo[j + 2])) then // x^3 x^2
        begin
  ...
  begin
    ...
    if (i >= 1) and (memo[i] = FracValida(memo[i])) and ((i < 2) or (memo[i - 2] <> '^')) then // x7 = 7x
      if memo[i - 1][1] in ['a'..'z', 'V'] then
      begin
        if (i >= 2) and (memo[i - 2] = '/') then
        begin
    ...
    if (i >= 3) and (memo[i] = FracValida(memo[i])) and (memo[i - 1] = '^') and (memo[i - 3] = '^') then // ^x^7 = ^7^x
      if memo[i - 2][1] in ['a'..'z', 'V'] then
      begin
        s := memo[i];
        memo[i] := memo[i - 2];
        memo[i - 2] := s;

        if (i >= 4) and (memo[i - 4] = fracValida(memo[i - 4])) then
        begin
          memo[i - 4] := fracPower(memo[i - 4], s);
          memo.Delete(i - 3);
          memo.Delete(i - 3);
        end;
        i := 0;
    ...
    if (i >= 1) and (memo[i - 1][1] in ['a'..'z', 'V']) then
      while (memo[j] <> '+') and (memo[j] <> '-') and (memo[j] <> '^') and (memo[j] <> '(') do
      begin
        if (j < memo.count) and (memo[j][1] in ['a'..'z', 'V']) and (memo[j - 1] <> '/')
          and ((i < 2) or (memo[i - 2] <> '^')) then // x x
  ...
  begin
    if i + 1 < memo.count then
      if Memo[i] = '*' then
        if (memo[i - 1] = FracValida(memo[i - 1])) and (memo[i + 1] = FracValida(memo[i + 1])) and ((i + 2 >= memo.count) or (memo[i + 2] <> '^')) then
        begin
          Memo[i - 1] := FracMul(Memo[i - 1], Memo[i + 1]);
          Memo.Delete(i);
          Memo.Delete(i);
...
//trim alphabet
  ...
  while i <= length(s) do
    ...
    else if (copy(s, i, 5) = 'binom') or (copy(s, i, 5) = 'floor') or (copy(s, i, 5) = 'gamma') then
    begin
      if s[i + 5] <> '(' then
      begin
        result := '';
    ...
    repeat
      ...
      begin
        ...
        begin
          ...
          else if (i >= 1) and (Memo[i - 1] = 'gamma') then
          begin
            j := i + 1;
            s := '';
            while Memo[j] <> ')' + inttostr(max) do
            ...
            Memo[i + 1] := Gamma(WithParenthesisProcess(true, s, LOGFILE, CANCELAR));
            while Memo[i + 2] <> ')' + inttostr(max) do
              Memo.Delete(i + 2);
          ...
          if copy(s, 1, 3) <> 'abs' then
            s := WithParenthesisProcess(true, s, LOGFILE, cancelar);
          if (i >= 1) and (copy(memo[i - 1], length(memo[i - 1]), 1) = '^') then
            if s = FracValida(s) then
            begin
              t := memo[i - 1];
              if t = '^' then
            ...
            begin
              ...
              if s[k] = '-' then
                inc(k);
              while not (s[k] in ['+', '-', '/', '^']) and (k <= length(s)) do
                inc(k);
              ...
              begin
                ...
                begin
                  t := memo[i - 2];
                  s := WithParenthesisProcess(true, Distribution(t, '^(' + s + ')', LOGFILE), LOGFILE, cancelar);
                  memo.Delete(i - 2);
                  memo.Delete(i - 2);
                  dec(i, 2);
                end
                ...
                begin
                  ...
                  while not (s[k] in ['+', '-', '/']) and (k <= length(s)) do
                    inc(k);
                  if (k <= length(s)) and (copy(s, 1, k) <> '1/') then
                  begin
                    delete(t, length(t), 1);
                    s := WithParenthesisProcess(true, Distribution(t, '^(' + s + ')', LOGFILE), LOGFILE, cancelar);
                    memo.Delete(i - 1);
              ...
              begin
                ...
                if (pos('/', s) > 0) and (pos('1/', s) <> 1) then
                  insert(memo[j + 1], s, pos('/', s));
              end
            end
          else if i >= 1 then
            if (memo[i - 1] = '/') and ((j + 1 >= memo.Count) or (copy(memo[j + 1], 1, 1) <> '^')) then
        ...
        //showmessage(memo.text);
          Memo.Delete(i + 2);
          Memo.Delete(i);
          if (i + 1 < memo.count) and (copy(memo[i + 1], 1, 1) = '^') and (s <> fracValida(s)) and (length(s) > 1) then
            memo[i] := '(' + s + ')'
          ...
          begin
            ...
            if (s <> 'binom') and (s <> 'abs') and (s <> 'gcd') and (s <> 'floor') and (s <> 'log') and (s <> 'gamma') then
            begin
              if (i >= 1) and (memo[i - 1] = '/') then
              begin
                s := Distribution(memo[i], '/' + s, LOGFILE);
                memo[i - 1] := '*';
                memo[i] := algebricalInverse(s);
              end
              else
          ...
          else if (i >= 1) and (memo[i - 1] = '/') and (memo[i] = fracValida(memo[i])) and (pos('/', memo[i]) > 0) then
          begin
            //eee
            memo[i - 1] := '*';
            s := fracDiv('1', memo[i]);
          ...
          begin
            ...
            else if (s = 'log') or (s = 'abs') or (s = 'gcd') or (s = 'floor') or (s = 'binom') or (s = 'gamma') then
              dec(j, 2)
            else
              memo[i] := Distribution(memo[i], '/' + s, LOGFILE);
...
begin
  ...
  begin
    ...
    begin
      ...
      if x[1] = '-' then
        inc(i);
      while (i <= length(x)) and not (x[i] in ['+', '-']) do
        inc(i);
      if (i <= length(x)) and (x[i] in ['+', '-']) then
        x := '(' + x + ')';
    end;
    ...
    else if length(y) > 1 then
      y := '(' + y + ')';
    result := result + x + '^' + y;
    if result[1] = '^' then
      result := '1' + result;
  end
  else if (copy(y, 1, 1) = '/') or (pos('+', y) = 0) and (pos('-', y) = 0) then
  begin
    flag := false;
    Result := copy(x, 1, 1);
    i := 2;
...
begin
  ...
  repeat
    ...
    if i > 0 then
    begin
      t := copy(Result, i + 1, length(Result));
    ...
    if s = fracValida(s) then
    begin
      Result := Distribution(s + '*', Result, LOGFILE);
      if t <> '' then
        t := Distribution(s + '*', t, LOGFILE);
    end
    else
    begin
    ...
    Result := WithParenthesisProcess(true, Result, LOGFILE, cancelar);
    if Result = '' then
      exit;
    if t <> '' then
    begin
...
begin
  ...
  if (pos(variable + '^', s) > 0) or (pos('^' + variable, s) > 0) then
  begin
...
begin
  ...
  result := copy(x, 1, i - 1);
  if result <> '' then
    if (result = copy(x3, 1, i3 - 1)) and not (result[1] in ['0'..'9']) then
...
begin
  ...
  if i = 0 then
    if s[1] = '-' then
      result := '-1/(' + AlgebricalOpposite(s) + ')'
    else
      result := '1/(' + s + ')'
  else if s[1] = '-' then
    result := '-' + copy(s, i + 1, length(s)) + '/' + copy(s, 2, i - 2)
  else
...
begin
  ...
  if (x[i - 1] in ['0'..'9']) and (y[1] in ['0'..'9']) then
    insert('*' + y, x, i)
  else
    insert(y, x, i);
...
procedure TFormPrincipal.Gamma1Click(Sender: TObject);
begin
  memoin.Text := 'gamma(6)';
end;
