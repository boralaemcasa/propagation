...
begin
  ...
  if FindFirst(nomeDir + edMask.Text, faArchive, search) = 0 then
    repeat
      ...
      if uppercase(copy(search.Name, length(search.Name) - 2, 3)) <> 'EXE' then //EXE 3
      if uppercase(copy(search.Name, length(search.Name) - 2, 3)) <> 'HTM' then //HTM 3
      if uppercase(copy(search.Name, length(search.Name) - 3, 4)) <> 'HTML' then //HTML 4
      if uppercase(copy(search.Name, length(search.Name) - 2, 3)) <> 'MP3' then //MP3 3
      if uppercase(copy(search.Name, length(search.Name) - 2, 3)) <> 'TXT' then //TXT 3
      begin
