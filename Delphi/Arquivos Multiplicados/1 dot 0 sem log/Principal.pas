...
begin
  ...
  if FindFirst(nomeDir + edMask.Text, faArchive, search) = 0 then
    repeat
      ...
      //djvu (4)
      //if uppercase(copy(search.Name, length(search.Name) - 3, 4)) <> 'DJVU' then
      begin
...
begin
  ...
  repeat
    ...
    caption := 'Deletar não repetidos: ' + inttostr(i) + ' x ' + inttostr(j);
    application.ProcessMessages;
	if cancelar then
	  exit;
...
begin
  ...
  repeat
    ...
    begin
      j := i + 1;
      repeat
        caption := 'Comparando ' + inttostr(i) + ' com ' + inttostr(j);
        application.processMessages;
