...
var bmp: TBitmap;
  ...
  begin
    ...
    while t < 7*pi do
    begin
      p.x := cos(t) * cos(pi/2 + t / 7);
      p.y := cos(t) * sin(pi/2 + t / 7);
      p.z := 0.5 * cos(pi/2 + t/7) - 0.5; // 4z(z+1)x^2 + (2z + 1)^2 y^2 = 0
      PontoXYZ(cos(t), sin(t), 0, clGreen);
      PontoXYZ(p.x, p.y,  p.z, clBlue);
      //PontoXYZ(p.x, p.y,  0, clRed);
      //PontoXYZ(p.x, p.y, -p.z, clRed);
      t := t + razao;
    end;
  end;
