...
INICIO:
   ...
   x := Valida(MemoEntrada.Lines.text);
   MemoEntrada.Lines.text := x;
   if SNumCompare(x, '2') < 0 then x := '0';//exit;
   n := Length(MemoEntrada.Lines.Text);
   fatorado := x + ' = ';
   ...
   repeat
      ...
      if (r = '0') and (x <> '0') then
         begin
            ...
            Memo.Lines.Add((x) + #32#247#32 + (ps) + ' = ' + (q));
            inc(novaVariavel);
            //Memo.Lines.SaveToFile('Linha ' + inttostr(novaVariavel) + '.txt');
            Application.ProcessMessages;
            x := q;
         end
      else
...
//                        else Memo.Lines.Add(espacos(Memo.Lines.Count) + ': ' + espacos(p) + ' > sqr(' + espacos(p2) + ')');
   ...
   IF NOT CANCEL THEN
      BEGIN
         x := Valida(MemoEntrada.Lines.text);
         MemoEntrada.Lines.Text := Subtrai(X, '1');
         MEMO.LINES.ADD('');
         GOTO INICIO;
      END;
end;
