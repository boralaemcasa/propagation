...
var
  ...
  m: integer;
  v: array[0..2, 0..1000] of double; // usage: v[0, m] embaixo, v[1, m] no meio, v[2, m] no fim
  inutil: boolean;
...
// v_m_n = v(t_n, x_m)
  ...
  assignfile(g, 'STRK18DA.txt');
  rewrite(g);
  assignfile(f, 'STRK18DB.txt');
  rewrite(f);
 ...
 // inicializar v[1, x], forall x
 // para D, use B para computar solução em n = 1
    t_n := t_n + k; // t_n = nk ???
    m := 0;
 ...
 // para BCD, x_M = 3, v_M_{n + 1} = v_{M - 1}_{n + 1}
    ...
    repeat // loop sobre t
      t_n := t_n + k; // t_n = nk ???
      m := 0;
	    ArmazenarV(2, m, u(t_n, x_ini));
      repeat // loop sobre x
        inc(m);
        x_m := x_ini + h * m;

     // Leapfrog (d) 1.3.4 com lambda = 0.8 // (v_m_{n + 1} - v_m_{n - 1})/k + a/(2 * h) * (v_{m + 1}_n - v_{m - 1}_n) = 0
     // a fórmula abaixo foi deduzida em Strik131.doc
        ArmazenarV(2, m, (h * v[0, m] - a * k * v[1, m + 1] + a * k * v[1, m - 1])/h );
        error := abs(sol(t_n, x_m) - v[2, m]);
        writeln(g, t_n, ' ; ', x_m, ' ; ', v[2, m], ' ; ', error);

        if (abs(x_m - 0) < 0.02) and (abs(t_n - 0.6) < 0.02) then
        begin
          writeln(f, h, ' ; ', v[2, m], ' ; ', error, ' ; ', error / errorAnt);
          errorAnt := error;
        end;
      until x_m >= x_fim;
      ...
      inc(m);
   // para BCD, x_M = 3, v_M_{n + 1} = v_{M - 1}_{n + 1}
      ArmazenarV(2, m, v[2, m - 1]); // a fórmula acima utiliza v[0, M + 1]
