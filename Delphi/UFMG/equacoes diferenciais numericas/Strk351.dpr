...
begin
  Result := 0;
  if (abs(t - 0) < 1e-14) or (abs(x - x_ini) < 1e-14) then
    Result := sin(pi * x);
end;
...
// v_m_n = v(t_n, x_m)
  ...
  assignfile(g, 'STRK351A.txt');
  rewrite(g);
  assignfile(f, 'STRK351B.txt');
  rewrite(f);
  ...
  repeat // loop sobre h
    ...
    repeat // loop sobre t
      ...
      inc(m);
   // ArmazenarV(1, m, ?); // a função abaixo consegue [1,2] em função de [0,1],[1,1],[1,0],[0,2],[0,0]
   // vamos completar a reta x = x_ini + 1 * h
   // utilizamos 1.3.1.B da mesma forma que foi utilizado em 1.3.1.D
   ...
   // a fórmula abaixo foi deduzida em Strik131.doc
     ...
     // Crank Nicolson
     // a fórmula abaixo foi deduzida em Strik131.doc
        ArmazenarV(1, m + 1, 4 * ( v[0, m] - v[1, m] ) / (a * lambda) + v[1, m - 1] - v[0, m + 1] + v[0, m - 1]);
        inc(m);
        x_m := x_ini + h * m;
        error := abs(sol(t_n, x_m) - v[1, m]);
        writeln(g, t_n, ' ; ', x_m, ' ; ', v[1, m], ' ; ', error);
