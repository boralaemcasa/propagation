...
// p(x) + s = 0 ==> a_n x^n = - s - Q
  ...
  begin
    ...
    dm.qryResetDetail.paramByName('L').asString := 'm%n' + n;
    dm.qryResetDetail.ExecSQL;
...
// \sum_1^{n-1} m_i q_i (p')^{2n - 2 - 2i} + (m_0 x + m_n) (p')^{2n - 3} = 0
  ...
  begin
    ...
    dm.qry2.sql.add('from polydetail');
    dm.qry2.sql.add('where p_who = ''q2''');
    dm.qry2.sql.add('   or p_who like ''m%n' + n + '''');
    dm.qry2.ExecSQL;
    dm.qry2.sql.clear;
    dm.qry2.sql.add('delete');
    dm.qry2.sql.add('from polymaster');
  ...
  begin
    ...
    dm.qryMinPower.paramByName('J').asString := Soma(n, '2');
    dm.qryMinPower.Open;
    if dm.qryMinPower.FieldByName('m').AsInteger > 0 then
    begin
      dm.qryPowerUp.ParamByName('p').AsString := dm.qryMinPower.fieldByName('m').AsString;
      dm.qryPowerUp.ParamByName('L').AsString := 'm%n' + n;
      dm.qryPowerUp.ParamByName('J').AsString := Soma(n, '2');
      dm.qryPowerUp.ExecSQL;
    end;
...
// extrair Ai0j0 ... Ai3j2
  ...
  for i := 0 to 3 do
    ...
    begin
      ...
      dm.qryExtractDetail.parambyname('origem').AsString := 'm' + inttostr(i) + 'n3';
      dm.qryExtractDetail.paramByName('who').AsString := 'mi' + inttostr(i) + 'j' + inttostr(j);
      //dm.qryExtractDetail.ExecSQL;
      dm.qryExtract0.paramByName('who').AsString := 'mi' + inttostr(i) + 'j' + inttostr(j);
      //dm.qryExtract0.ExecSQL;
...
var
  i, j, k, L, max, minimo, count: integer;
...
begin
  ...
  dm.qryResetDetail.paramByName('L').asString := 'mi%';
  dm.qryResetDetail.ExecSQL;
  dm.qryResetMaster.paramByName('L').asString := 'mi%';
  dm.qryResetMaster.ExecSQL;

  dm.qryResetDetail.paramByName('L').asString := 'm%n' + inttostr(N) + 'sol';
  dm.qryResetDetail.ExecSQL;
...
// extrair Ai0j0 ... Ai3j2
  for i := 0 to N do
    ...
    begin
      ...
      dm.qryExtractMaster.paramByName('p').AsInteger := j;
      dm.qryExtractMaster.ExecSQL;
      dm.qryExtractDetail.parambyname('origem').AsString := 'm' + inttostr(i) + 'n' + inttostr(N);
      dm.qryExtractDetail.paramByName('who').AsString := 'mi' + inttostr(i) + 'j' + inttostr(j);
      dm.qryExtractDetail.ExecSQL;
      dm.qryExtract0.paramByName('who').AsString := 'mi' + inttostr(i) + 'j' + inttostr(j);
      dm.qryExtract0.ExecSQL;
      ...
      a[i, j].mywho := 'mi' + inttostr(i) + 'j' + inttostr(j);
      dm.qryCounter.ParamByName('who').AsString := 'mi' + intToStr(i) + 'j' + intToStr(j);
      dm.qryCounter.Open;
      a[i, j].list.text := dm.qryCounter.fieldByName('c').AsString;
      dm.qryCounter.Close;
    end;
  ...
  repeat
    ...
    begin
      ...
      begin
        ...
        dm.qryResetDetail.paramByName('L').asString := 'mi' + inttostr(i) + 'j' + inttostr(j);
        dm.qryResetDetail.ExecSQL;
...
//agora vamos calcular os emmys
  ...
  mj := a[k, 0]; // don't free this emmy
  mk := a[j, 0].TimesConstantToDisk('U' + u, '-1');
  u := Soma(u, '1');
  ...
  begin
    ...
    m[simples[i]] := p3.TimesConstantToDisk('U' + u, '-1');
    u := Soma(u, '1');
    p1.free(false);
    p2.free(false);
    p3.free(false);
