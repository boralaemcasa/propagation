...
begin
  ...
  dm.qryCoef.Open;
  dm.qryCoef.First;
  result.list.Text := dm.qryCoef.fieldByName('i').AsString;
  while not dm.qryCoef.Eof do
  begin
    dm.qryUpdateCoef.ParamByName('pc').AsString := FracMul(dm.qryCoef.fieldByName('coef').AsString, dm.qryCoef.fieldByName('coef2').AsString);
    dm.qryUpdateCoef.ParamByName('pnew').AsString := who;
    dm.qryUpdateCoef.ParamByName('pi').AsString := dm.qryCoef.fieldByName('i').AsString;
    dm.qryUpdateCoef.ExecSQL;

    if (dm.qryCoef.RecNo mod 10 = 0) and (dm.qryCoef.recordcount > 1) then
      if dm.qryCoef.RecNo < dm.qryCoef.RecordCount then
      begin
        if memo.Lines.Count >= 10240 then
          memo.clear;
        memo.lines.add('mult (I) ' + inttostr(dm.qryCoef.RecNo) + '/' + inttostr(dm.qryCoef.RecordCount) + ' = ' +
          inttostr((dm.qryCoef.RecNo) * 100 div dm.qryCoef.RecordCount) + ' %');
        application.processMessages;
      end;
    dm.qryCoef.Next;
  end;
...
begin
  ...
  begin
    if dm.qrySimplify.Tag <> dim then
    begin
      dm.qrySimplify.SQL.Clear;
      dm.qrySimplify.SQL.add('select i, coef');
      dm.qrySimplify.SQL.add('from polymaster m');
      for i := 1 to dim do
        dm.qrySimplify.SQL.add('  , polydetail d' + inttostr(i));
      dm.qrySimplify.SQL.add('where who = :w');
      for i := 1 to dim do
      begin
        dm.qrySimplify.SQL.add('  and d' + inttostr(i) + '.p_who = m.who');
        dm.qrySimplify.SQL.add('  and d' + inttostr(i) + '.p_i = m.i');
        dm.qrySimplify.SQL.add('  and d' + inttostr(i) + '.j = ' + quotedstr(inttostr(i)));
        dm.qrySimplify.SQL.add('  and d' + inttostr(i) + '.power = :p' + inttostr(i));
      end;
      dm.qrySimplify.Tag := dim;
    end;

    coef := '0';

    dm.qrySimplify.ParamByName('w').AsString := mywho;
    for i := 1 to dim do
      dm.qrySimplify.ParamByName('p' + inttostr(i)).AsString := dm.qryMinI.fieldByName('p' + inttostr(i)).AsString;
    dm.qrySimplify.Open;
    dm.qrySimplify.First;
    if not dm.qrySimplify.IsEmpty then
      while not dm.qrySimplify.eof do
      begin
        coef := FracAdd(dm.qrySimplify.fieldByName('coef').asstring, coef);

        dm.qryDelDetail.ParamByName('p1').AsString := mywho;
        dm.qryDelDetail.ParamByName('p3').AsString := dm.qrySimplify.fieldbyName('i').asString;
        dm.qryDelDetail.ExecSQL;

        dm.qryDelMaster.ParamByName('p1').AsString := mywho;
        dm.qryDelMaster.ParamByName('p3').AsString := dm.qrySimplify.fieldbyName('i').asString;
        dm.qryDelMaster.ExecSQL;

        dm.qrySimplify.Next;
      end;
    dm.qrySimplify.Close;

    if coef <> '0' then
    begin
      dm.qryInsMaster.ParamByName('p1').asString := mywho;
      dm.qryInsMaster.ParamByName('p2').asInteger := dm.qryMinI.fieldByName('m').AsInteger;
      dm.qryInsMaster.ParamByName('p3').asString := coef;
      dm.qryInsMaster.ExecSql;
      ...
      //salve os zeros!
      begin
        dm.qryInsDetail.ParamByName('P1').AsString := mywho;
        dm.qryInsDetail.ParamByName('P2').asInteger := dm.qryMinI.fieldByName('m').AsInteger;
        dm.qryInsDetail.ParamByName('P3').AsString := intToStr(i);
        dm.qryInsDetail.ParamByName('P4').AsSmallInt := dm.qryMinI.fieldByName('p' + inttostr(i)).AsInteger;
        dm.qryInsDetail.ExecSql;
      end;
    end;
