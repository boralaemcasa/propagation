  function [obj,x,y,iter,evolx,xb1,B] = revised_simplex(c,A,b,eps_x,B)
...
% [obj,x,y] = revised_simplex(c,A,b,eps_x,B)
% eps_x is a suitable optimality tolerance, say 1e-3
...
% % %           fprintf(' Simplex  Optimal objective value is  = %f\n',c'*x);
% % %           disp('  ------------------------------------------------------  ');
           x = zeros(n,1);
           x(B) = xB;
           obj = c'*x;
           evolx(:,min(iter,1000000)) = x;
           return;
        end;
