...
X = [];
for k = 34:length(y)
    X = [X; -y(k-1) -y(k-2) -y(k-3) -y(k-4) -y(k-5) u(k - 29) u(k - 30) u(k - 31) u(k - 32) u(k - 33)];
end
...
z2 = y(1:33)';
for k = 34:length(y)
    z2(k) = - theta(1) * z2(k-1) - theta(2) * z2(k-2) - theta(3) * z2(k-3) - theta(4) * z2(k-4) - theta(4) * z2(k-4) - theta(5) * z2(k-5) + theta(6) * u(k - 29) + theta(7) * u(k - 30) + theta(8) * u(k - 31) + theta(9) * u(k - 32) + theta(10) * u(k - 33);
end

figure(4)
plot(tt,y,tt(1:205),z2(1:205))
set(gca,'FontSize',18)
xlabel('t')
ylabel('y')
